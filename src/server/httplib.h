...
namespace httplib {
    ...
    private:
        ...
        std::atomic<socket_t> svr_sock_;  //监听socket
        std::atomic<int> epfd;           //epoll fd
...
//读写的epoll  每次返回指定的描述符
        inline int select_read(socket_t sock, time_t sec, time_t usec) {
            (void) usec;
            epoll_event evs[ANY-1];
            int ret = epoll_wait(g_ep, evs, 1, sec);
            for (int i = 0; i < ret; ++i) {
                if (evs[i].data.fd == sock) {
                    return evs[i].data.fd;
                }
            }
...
//返回特定的描述符
        inline int select_read(socket_t sock, time_t sec, time_t usec) {
            ...
            /*epoll_event evs[ANY - 1];
            int ret = epoll_wait(epfd, evs, ANY - 1, tv.tv_sec);
            for (int i = 0; i < ret; ++i) {
                if (evs[i].data.fd == sock) {
                    return evs[i].data.fd;
                }
        ...
        inline bool read_and_close_socket(socket_t sock, size_t keep_alive_max_count,
            ...
            if (keep_alive_max_count > 0) {  //到达最大保活线程数
...
#define SO_OPENTYPE 0x7008
        ...
        template<typename T>
        inline int write_headers(Stream &strm, const T &info) {
            auto write_len = 0;
...
// Request implementation
    ...
    inline void Request::setfd(int f) {
        fd = f;
    }

    inline int Request::getfd() const {
        return fd;
    }
...
//监听   ------->   ip 和  端口    入口        flags 默认为0
    inline bool Server::listen(const char *host, int port, int socket_flags) {
        if (bind_internal(host, port, socket_flags) < 0) return false;   //bind_internal  内部会调用设置svr_socket
        epfd = epoll_create(ANY);  //svr_sock创建好后   建立epoll
        g_ep = epoll_create(ANY);
        if (epfd < 0 || g_ep < 0) {
            return false;
        }
        return listen_internal();  //监听 svr_sock
    }
